          ...
          {CLIENT_NEEDS.map((need, i) => {
            ...
            return (
              <motion.a
                ...
                className="group flex items-start gap-4 p-5 rounded-xl bg-white border border-slate-200 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-100/50 transition-all duration-300"
                style={{ ['--hover-color' as string]: need.color }}
              >
                <Icon className="w-5 h-5 mt-0.5 shrink-0 transition-colors" style={{ color: need.color }} />
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-semibold text-slate-950 group-hover:text-blue-700 transition-colors">{need.title}</h3>
